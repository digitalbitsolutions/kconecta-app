...
- Goal: surface manager-safe pending handoff and contract actions in the dashboard so operational follow-up no longer depends on manually opening downstream flows to discover blocked work.
- Architect: define an additive pending-actions contract, dashboard state map, and navigation ownership for handoff and contract action items.
...
- Mobile: render a pending-actions dashboard section with tappable rows that route into the relevant handoff or contract detail surfaces while preserving current dashboard summary and priorities behavior.
